...
export function LoanErrorHandler (auth) {
  return (errorObject, notify, cb = () => {}) => {
  const { message } = errorObject;
  const isTokenExpired = errorObject?.toLowerCase?.().includes?.("unauth") || errorObject?.data?.toLowerCase?.().includes?.("token");
  const msg = isTokenExpired ? "Session Expired, please log in." :
    message?.toLowerCase?.().includes?.("timeout") ? "Service Timeout, Please Retry" :
    message?.toLowerCase?.().includes?.("network") ? "Network Error!" : "Something went wrong!";
